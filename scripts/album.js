...
var setCurrentAlbum = function(album) {
    ...
    for (var i = 0; i < album.songs.length; i++) {
        var $newRow = createSongRow(i + 1, album.songs[i].title, album.song[i].duration);
...
window.onload = function() {
    ...
    songListContainer.addEventListener('mouseover', function(event) {
      if (event.target.parentElement.className === 'album-view-song-item') {
          var numElement = event.target.parentElement.querySelector('.song-item-number');

          if (numElement.getAttribute('data-song-number') !== currentlyPlayingSong) {
              numElement.innerHTML = playButtonTemplate;
          }
      }
    });
